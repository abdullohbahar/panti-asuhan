                ...
                <div class="card-header">
                    <div class="row justify-content-between">
                        ...
                        <div class="col-sm-12 col-md-4 text-right">
                            ...
                            <a href="{{ route('export.donasi.transfer.pdf') }}" class="btn btn-danger btn-sm"><b><i
                    ...
                    <div class="row">
                        <div class="col-sm-12 col-md-12 col-lg-2 col-xl-2">
                            <div class="form-group">
                                <input type="date" wire:model.defer="date1" class="form-control" name=""
                                    id="">
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-12 col-lg-2 col-xl-2">
                            <div class="form-group">
                                <input type="date" wire:model.defer="date2" class="form-control" name=""
                                    id="">
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-12 col-lg-2 col-xl-2">
                            <div class="form-group">
                                <button wire:click="search" id="search" class="btn btn-info btn-block">Filter
                                    Data</button>
                            </div>
                        </div>
                        <div class="col-sm-12 col-md-12 col-lg-2 col-xl-2">
                            <div class="form-group">
                                <a href="{{ route('data.donasi.transfer') }}" class="btn btn-warning btn-block">Reset
                                    Filter</a>
                            </div>
                        </div>
                    </div>
                                ...
                                <thead>
                                    <tr>
                                        ...
                                        @if (auth()->user()->role == 'admin-yayasan' ||
                                                ...
                                                Auth()->user()->role == 'bendahara-yayasan' ||
                                                Auth()->user()->role == 'admin-donasi')
                                    ...
                                    @foreach ($donations as $index => $donation)
                                        <tr>
                                            ...
                                            <td data-label="Nominal">
                                                {{ 'Rp ' . number_format($donation->pemasukan, 2, ',', '.') }}</td>
                                            <td data-label="No Rek">{{ $donation->norek }}</td>
                                            <td data-label="Bank">{{ $donation->bank }}</td>
                                            <td data-label="Tanggal Donasi">
                                                {{ date('d-m-Y', strtotime($donation->tanggal_donasi)) }}</td>
                                            <td data-label="Aksi">
                                                ...
                                                @if (auth()->user()->role == 'admin-yayasan' ||
                                                        ...
                                                        Auth()->user()->role == 'admin-donasi')
                                                    <button id="edit"
                                                        wire:click="show('{{ $donation->id }}','{{ $donation->donatur_id }}')"
                                                        data-jenis="{{ $donation->jenis_donasi }}"
                                                        class="btn btn-warning btn-sm" data-toggle="modal"
                                                        data-target="#modal-edit-donation" data-toggle="tooltip"
                                                        data-placement="top" title="Ubah Donasi"><i
                                                            class="fas fa-pencil-alt"></i></button>
                                                    <button wire:click="deleteConfirmation('{{ $donation->id }}')"
                                                        class="btn btn-danger btn-sm" data-toggle="tooltip"
                                                        data-placement="top" title="Hapus Donasi"><i
                                                            class="fas fa-trash-alt"></i></button>
                                                    <a onclick="openWindowPopup('/print-invoice-donation-transfer/{{ $donation->id }}', 1200, 800)"
                                                        class="btn btn-primary btn-sm" data-toggle="tooltip"
                                                        data-placement="top" title="download bukti"><i
