<div>
  ...
  <section class="content-header">
    <div class="container-fluid">
      <div class="row mb-2">
        <div class="col-sm-6">
          <h1>Data Surat Keluar Yayasan</h1>
        </div>
        <div class="col-sm-6">
          <ol class="breadcrumb float-sm-right">
            <li class="breadcrumb-item"><a href="{{ route('dashboard') }}">Home</a></li>
            <li class="breadcrumb-item active">Data Surat Keluar Yayasan</li>
          </ol>
        </div>
      </div>
    </div><!-- /.container-fluid -->
                            ...
                            <tbody>
                                @foreach ($letters as $index => $letter)
                                    <tr>
                                        ...
                                        <td data-label="File"><button wire:click="download('{{ $letter->file }}','{{ $letter->perihal }}')" class="btn btn-sm btn-success">Unduh Surat</button></td>
                                        @if (auth()->user()->role == 'admin-yayasan' || Auth()->user()->role == 'ketua-yayasan' || Auth()->user()->role == 'sekertariat-yayasan')
                                            <td data-label="Aksi">
                                                ...
                                                <button wire:click="deleteConfirmation('{{ $letter->id }}','{{ $letter->file }}')" class="btn btn-danger btn-sm" data-toggle="tooltip" data-placement="top" title="Hapus Donasi"><i class="fas fa-trash-alt"></i></button>
                                            </td>
                                        @endif
                                    </tr>
                                @endforeach
